...
module.exports.AuthFlow = class AuthFlow {
  constructor() {
    ...
    this.notifier.setAuthorizationListener(function (request, response, error) {
      if (response) {
        ...
        _this
          .makeRefreshTokenRequest(response.code, codeVerifier)
          .then(function (result) {
            return _this.performWithFreshTokens();
          })
          .then(function () {
            _this.processData();
          });
  ...
  fetchServiceConfiguration = function () {
    const _this = this;
    return AppAuth.AuthorizationServiceConfiguration.fetchFromIssuer(openIdConnectUrl, requestor).then(function (
      response
    ) {
  ...
  makeRefreshTokenRequest = function (code, codeVerifier) {
    ...
    return this.tokenHandler
      ...
      .then(function (response) {
        ...
        fs.writeFileSync(
          path.join(__dirname, "oauthclient.json"),
  ...
  performWithFreshTokens = function () {
    ...
    return this.tokenHandler.performTokenRequest(this.configuration, request).then(function (response) {
      _this.accessTokenResponse = response;
  ...
  getCourses = function (accessToken) {
    ...
    return new Promise((resolve, reject) => {
      _this
        .makeAPIRequest(accessToken, "https://classroom.googleapis.com/v1/courses")
        .then((coursesList) => {
          resolve(coursesList);
        })
        .catch((error) => {
  ...
  getUserInfo = function (accessToken) {
    ...
    return new Promise((resolve, reject) => {
      _this
        .makeAPIRequest(accessToken, "https://classroom.googleapis.com/v1/userProfiles/me")
        .then((userInfo) => {
          resolve(userInfo);
        })
        .catch((error) => {
  ...
  getCourseWork = function (accessToken, courseId) {
    ...
    return new Promise((resolve, reject) => {
      _this
        .makeAPIRequest(accessToken, `https://classroom.googleapis.com/v1/courses/${courseId}/courseWork`)
        .then((courseWork) => {
          resolve(courseWork);
        })
        .catch((error) => {
    ...
    return new Promise((resolve, reject) => {
      Promise.all([_this.getCourses(accessToken), _this.getUserInfo(accessToken)]).then((coursesAndUserInfo) => {
        ...
        Promise.all(courseWorkPromises).then((allCourseWork) => {
          const allDueCourseWork = allCourseWork
            .filter((course) => course.courseWork != undefined)
            .map((course) => course.courseWork)
            .flat()
            .filter((e) => {
              ...
              return (
                new Date(e.dueDate.year, e.dueDate.month - 1, e.dueDate.day, e.dueTime.hours - 7, e.dueTime.minutes) >
                Date.now()
              );
            });
